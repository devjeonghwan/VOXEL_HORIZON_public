...
enum SHADE_TYPE
{
	...
	SHADE_TYPE_VERTEX_LIGHT_FIXED		= 0x00000001,
	SHADE_TYPE_MIRROR_WATER				= 0x00000002,
	SHADE_TYPE_LIGHT_MAP				= 0x00000003,
	SHADE_TYPE_MIRROR					= 0x00000004
...
class CMeshFlags
{
	DWORD				m_dwFlags;
public:
	BOOL				IsDisableZBubfferWrite() { return (m_dwFlags & WRITE_ZBUFFER_MASK)>>31;}
	void				DisableZBufferWrite()	{ m_dwFlags |= WRITE_ZBUFFER_MASK;}
	void				EnableZBufferWrite()	{m_dwFlags &= WRITE_ZBUFFER_MASK_INVERSE;}

	int					GetRenderZPriorityValue() 
	{
		// 최상위 1비트는 z버퍼쓰기 가능/불가능, 그 다음 7비트는 렌더링 순서로 사용한다.
		// 7비트중 최상위비트는 싸인비트로 사용한다.
		int		iZOrder =  (m_dwFlags & RENDER_ZPRIORITY_MASK)>>24;
	...
	void				SetRenderZPriorityValue(int iZOrder) 
	{
		...
		m_dwFlags = (m_dwFlags & RENDER_ZPRIORITY_MASK_INVERSE) | (iZOrder<<24);
	}

	SHADE_TYPE			GetShadeType() {return (SHADE_TYPE)(m_dwFlags & SHADE_TYPE_MASK);}
	void				SetShadeType(SHADE_TYPE type) {m_dwFlags = (m_dwFlags & SHADE_TYPE_MASK_INVERSE) | type;}

	

	TRANSFORM_TYPE		GetTransformType() {return (TRANSFORM_TYPE)(m_dwFlags & TRANSFORM_TYPE_MASK);}
	void				SetTransformType(TRANSFORM_TYPE type) {m_dwFlags = (m_dwFlags & TRANSFORM_TYPE_MASK_INVERSE) | type;}

	RIGID_TYPE			GetRigidType()  {return (RIGID_TYPE)(m_dwFlags & RIGID_TYPE_MASK);}
	void				SetRigidType(RIGID_TYPE type) {m_dwFlags = (m_dwFlags & RIGID_TYPE_MASK_INVERSE) | type;}


	PICK_ENABLE_TYPE	GetPickEnable() {return (PICK_ENABLE_TYPE)(m_dwFlags & PICK_ENABLE_TYPE_MASK);}
	void				SetPickEnable(PICK_ENABLE_TYPE type) {m_dwFlags = (m_dwFlags & PICK_ENABLE_TYPE_MASK_INVERSE) | type;}

	
	CMeshFlags()			{m_dwFlags = 0;}
};
...
class CLightFlags
{
	DWORD			m_dwFlags;
public:
	DYNAMIC_LIGHT_APPLY_TYPE	GetDynamicLightType() {return (DYNAMIC_LIGHT_APPLY_TYPE)(m_dwFlags & DYNAMIC_LIGHT_APPLY_TYPE_MASK);}
	void						SetDynamicLightType(DYNAMIC_LIGHT_APPLY_TYPE type) {m_dwFlags = (m_dwFlags & DYNAMIC_LIGHT_APPLY_TYPE_MASK_INVERSE) | type;}
	CLightFlags()				{m_dwFlags = 0;}
};
